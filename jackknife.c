...
int size;
int binSize;
double *dataPlaquette;
double *dataChargeSq;
double *dataChargeEvenOdd;
double *dataPlaquetteBin;
double *dataChargeSqBin;
double *dataChargeEvenOddBin;

void NewDataset(char *paramFname)
{
    ...
    therm = sweeps*THERMRATIO;
    size = sweeps - therm;
    binSize = ( size + NBINS - 1 ) / NBINS;

    if (getPlaquette)
    {
        FILE *plaquetteFile;
        assert( plaquetteFile = fopen(plaquetteHistory, "r") );
        dataPlaquette = malloc(size*sizeof(double));
        dataPlaquetteBin = calloc(NBINS,sizeof(double));
        for (int i=0; i<therm; i++) assert( fscanf(plaquetteFile, "%*f") == 0 );
        for (int i=0; i<size; i++)
        {
            double temp;
            assert( fscanf(plaquetteFile, "%lf", temp) == 1 );
            dataPlaquette[i] = temp;
            dataPlaquetteBin[i/binSize] += temp;
        }
        fclose(plaquetteFile);

        for (int i=0; i<NBINS-1; i++) dataPlaquetteBin[i] /= binSize;
        dataPlaquetteBin[NBINS-1] /= size%binSize;
    }
    ...
    if (getChargeSq) 
    {
        ...
        dataChargeSq = malloc(size*sizeof(double));
        dataChargeSqBin = calloc(NBINS,sizeof(double));
        ...
        for (int i=0; i<size; i++) 
        {
            ...
            temp = gsl_pow_2(temp);
            dataChargeSq[i] = temp;
            dataChargeSqBin[i/binSize] += temp;
        }
        fclose(chargeSqFile);

        for (int i=0; i<NBINS-1; i++) dataChargeSqBin[i] /= binSize;
        dataChargeSqBin[NBINS-1] /= size%binSize;
    }
    ...
    if (getChargeEvenOdd)
    {
        ...
        dataChargeEvenOdd = malloc(size*sizeof(double));
        ...
        {
            double temp;
            assert( fscanf(chargeEvenOddFile, "%lf", &temp) == 1 );
            dataChargeEvenOdd[i] = temp;
            dataChargeEvenOddBin[i/binSize] += temp;
        }
        fclose(chargeEvenOddFile);

        for (int i=0; i<NBINS-1; i++) dataChargeEvenOddBin[i] /= binSize;
        dataChargeEvenOddBin[NBINS-1] /= size%binSize;
    }
}
...
void DeleteDataset()
{
    ...
    if (getChargeSq) 
    {
        free(dataChargeSq) = malloc(size*sizeof(double));
        Read(dataChargeSq, chargeSqHistory);
    }

    if (getChargeEvenOdd)
    {
       dataChargeEvenOdd = malloc(size*sizeof(double));
       Read(dataChargeEvenOdd, chargeEvenOddHistory);
    }



double *Bunching(FILE *output, FILE *input)
{
